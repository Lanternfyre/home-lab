...
| | |
|---|---|
| Current token | ⚠️ **HISTORIC ROW — rotated 2026-09-06.** This described `k3sblog`: 7 characters, a dictionary word plus a suffix. It is dead; `server-bootstrap` returns 401 for it on all three servers. Servers and agents now hold *different* tokens, both in 1Password |
...
| Who manages it | ⚠️ **HISTORIC ROW.** Said "nobody". Now: `roles/k3s_config` renders `token-file` / `agent-token-file`, both join playbooks pick their source by role, `71-rotate-cluster-token.yml` is the runbook, and both tokens live in 1Password |
| Exposure | ⚠️ **HISTORIC ROW.** `k3sblog` was committed in plaintext in `gitops/argo-install.md` in a **public** repo. That file now reads `--token "$K3S_TOKEN"`. The old value remains in git history permanently — which is *why* it was rotated, not a residual fault |
...
k8s-7                 k3s-agent.service   agent  (joined 2026-09-05)
edge-1.edge           k3s-agent.service   agent  (public VPS, joined over
                                                  WireGuard; holds the AGENT
                                                  token only)
```
...
1. **Strength.** Generate it, do not choose it.
   🔴 **`openssl rand -hex 24`, NOT `k3s token generate`** — corrected
   2026-09-06 during the real rotation. This line used to recommend
   `k3s token generate`, which emits a *bootstrap* token
   (`xxxxxx.xxxxxxxxxxxxxxxx`) that k3s then **rejects**:
   `failed to normalize server token; must be in format
   K10<CA-HASH>::<USERNAME>:<PASSWORD>`. See "Secrets are prompted" below,
   which is where the corrected form was first written down.
...
3. **A delivery path that is not the unit file.**
   ⚠️ **Corrected 2026-09-06.** This used to say `--token` sits in `ExecStart`.
   It does not — the servers pass no arguments at all. The override was
   `K3S_TOKEN=` in each **agent's** `k3s-agent.service.env`, which beats
   `config.yaml` exactly as a CLI flag would. Normalising the *`.env`* was the
   blocker; normalising `ExecStart` never was. See "Where it actually lives"
   above, and `0b331fe`, which stripped it.

✅ All three are now done, which is what made the 2026-09-06 rotation possible.
⚠️ `30-upgrade.yml` wipes the agent `.env` on every upgrade — that is why
`token-file:` in `config.yaml` is the durable target rather than the env file.

---

## Target state

- Token generated by `openssl rand -hex 24`, never hand-chosen and **never**
  `k3s token generate` (see the correction above — it emits a bootstrap token
  k3s rejects)
- **Two** tokens, not one: a server token and an `agent-token`, so an agent
  credential cannot join a server
